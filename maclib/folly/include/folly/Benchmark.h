...
struct BenchmarkSuspender {
  BenchmarkSuspender() {
    CHECK_EQ(0, clock_gettime((clockid_t)detail::DEFAULT_CLOCK_ID, &start));
  }
...
private:
  void tally() {
    timespec end;
    CHECK_EQ(0, clock_gettime((clockid_t)detail::DEFAULT_CLOCK_ID, &end));
...
addBenchmark(const char* file, const char* name, Lambda&& lambda) {
  auto execute = [=](unsigned int times) {
    ...
    auto const r1 = clock_gettime((clockid_t)detail::DEFAULT_CLOCK_ID, &start);
    niter = lambda(times);
    auto const r2 = clock_gettime((clockid_t)detail::DEFAULT_CLOCK_ID, &end);
